...
def index_documents(documents: List[integrations_api.BasicDocument]):
    db_documents = []
...
def search_documents(query: str, top_k: int) -> List[Paragraph]:
    ...
    index = Index.get()
    top_k_ids = index.search(query_embedding, 100)

    # Get the paragraphs from the database
    session = Session()
    paragraphs = session.query(Paragraph).filter(Paragraph.id.in_(top_k_ids[0])).all()
    ...
    large_scores = cross_encoder_large.predict([(query, c['paragraph'].content) for c in candidates])[0]
    candidates = [{
