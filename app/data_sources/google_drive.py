...
class GoogleDriveDataSource(BaseDataSource):
    ...
    def feed_new_documents(self) -> None:
        files = self.drive.files().list(fields='files(kind,id,name,mimeType,owners,webViewLink,createdTime,parents)').execute()
        files = files['files']
        documents = []
        ...
        for file in files:
            if file['mimeType'] == 'application/vnd.google-apps.document' and file['kind'] == 'drive#file':
                id = file['id']
                ...
                documents.append(BasicDocument(
                    ...
                    timestamp=datetime.strptime(file['createdTime'], "%Y-%m-%dT%H:%M:%S.%fZ")
                ))
